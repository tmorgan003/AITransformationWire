...
The Settings page's "Run briefing now" button runs `app.run` in a background thread, so there's no need to touch the CLI day-to-day.
...
This means:
- Iterating on `render.py`, `summarize.py`, the prompt, or anything else code-side re-runs instantly against the same cached data, with no network calls.
...
- Every source entry needs a `homepage`, used both for the clickable per-article source tag and the Settings/Resources source listing.
